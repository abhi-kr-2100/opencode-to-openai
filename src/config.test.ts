import { describe, expect, test } from "bun:test";
import { displayAddress, loadConfig } from "./config.ts";

describe("displayAddress", () => {
  test("maps wildcard binds to a loopback address", () => {
    expect(displayAddress("0.0.0.0")).toBe("127.0.0.1");
    expect(displayAddress("::")).toBe("127.0.0.1");
    expect(displayAddress("::0")).toBe("127.0.0.1");
    expect(displayAddress("0:0:0:0:0:0:0:0")).toBe("127.0.0.1");
  });

  test("keeps concrete addresses unchanged", () => {
    expect(displayAddress("127.0.0.1")).toBe("127.0.0.1");
    expect(displayAddress("localhost")).toBe("localhost");
    expect(displayAddress("::1")).toBe("::1");
  });
});

describe("loadConfig", () => {
  test("defaults host and port", () => {
    expect(loadConfig({})).toEqual({ host: "127.0.0.1", port: 8000 });
  });

  test("reads PORT and HOST from env", () => {
    expect(loadConfig({ PORT: "9000", HOST: "0.0.0.0" })).toEqual({
      host: "0.0.0.0",
      port: 9000,
    });
  });

  test("trims whitespace", () => {
    expect(loadConfig({ PORT: " 8123 ", HOST: " localhost " })).toEqual({
      host: "localhost",
      port: 8123,
    });
  });

  test("accepts boundary ports", () => {
    expect(loadConfig({ PORT: "0" }).port).toBe(0);
    expect(loadConfig({ PORT: "65535" }).port).toBe(65_535);
  });

  test("throws for a non-numeric port", () => {
    expect(() => loadConfig({ PORT: "abc" })).toThrow(/invalid PORT/);
  });

  test("throws for an out-of-range port", () => {
    expect(() => loadConfig({ PORT: "-1" })).toThrow(/invalid PORT/);
    expect(() => loadConfig({ PORT: "65536" })).toThrow(/invalid PORT/);
  });
});
